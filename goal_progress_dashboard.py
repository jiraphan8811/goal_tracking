...
def log_selector_labels(goals: pd.DataFrame, logs: pd.DataFrame) -> Dict[str, str]:
    """Return unique, newest-first labels for the Edit/Delete Log selector.

    Google Sheets appends new rows at the bottom. The selector must therefore not
    rely on sheet row position. We sort by log_date, created_at, and original row
    order, then use log_id as the stable record key. The short log_id suffix is
    included in the visible label so two similar logs on the same day do not
    ...
    merged["log_date"] = pd.to_datetime(merged.get("log_date"), errors="coerce")
    merged["created_at"] = pd.to_datetime(merged.get("created_at"), errors="coerce")
    merged = merged.dropna(subset=["log_date"])

    # Newest logs first, even when Google Sheets stores the newest row at the bottom.
    merged = merged.sort_values(
        by=["log_date", "created_at", "_row_order"],
        ascending=[False, False, False],
        na_position="last",
    )

    labels: Dict[str, str] = {}
    for _, r in merged.head(300).iterrows():
        log_id = str(r.get("log_id", "")).strip()
        if not log_id:
            continue

        log_day = r["log_date"].date()
        created_at = r.get("created_at")
        created_txt = "" if pd.isna(created_at) else f" @ {created_at.strftime('%H:%M')}"
        achievement = str(r.get("achievement", "") or "").strip()[:45]
        id_suffix = log_id[-6:]

        label = (
            f"{log_day}{created_txt} | {r.get('title', 'Unknown')} | "
            f"{float(r.get('hours_spent', 0) or 0):.2f}h | {achievement} | #{id_suffix}"
        )
        labels[label] = log_id
    return labels
